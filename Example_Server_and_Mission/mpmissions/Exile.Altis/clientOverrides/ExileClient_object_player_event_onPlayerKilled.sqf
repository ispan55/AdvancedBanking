...
if (_wallet > 0) then {
    ...
    _pile = createVehicle ["Land_Money_F", [(_pos select 0)+0.2,(_pos select 1)+0.2,0], [], 0, "NONE"];
    _pile setVariable ["DroppedAmount",_wallet,true];
	_pile setVariable ["PileOwner",_pileOwner,true];
    ["updateWallet",ExileClientSessionId,[(getPlayerUID player),0]] call AdvBanking_Client_PacketMuncher;
